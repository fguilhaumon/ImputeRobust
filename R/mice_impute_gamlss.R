...
mice.impute.gamlss <- function(y, ry, x, family = NO, n.ind.par = 2,
  ...
  while (sum(is.na(imputed.values)) > 0 & count < n_tries) {
    if (count > 0) message("Problem when fitting gamlss (", count,  "/", n_tries,"\n")
    if (count == 0) message("First fit\n")
    imputed.values <- fitted.gam(...)
    count <- count + 1
  }
...
### EXP, GA, IGAMMA, LNO, IG, WEI, WEI2, WEI3, LOGNO, LOGNO2, BCTo, PARETO2o, BCPEo
...
#' @rdname mice.impute.gamlss
#' @export
mice.impute.gamlssWEI3 <- function(y, ry, x, fitted.gam = NULL, EV = TRUE, ...) {
  Call <- match.call(expand.dots = TRUE)
  Call[["family"]] <- WEI3
...
#' @rdname mice.impute.gamlss
#' @export
mice.impute.gamlssLOGNO <- function(y, ry, x, fitted.gam = NULL, EV = TRUE, ...) {
  Call <- match.call(expand.dots = TRUE)
  Call[["family"]] <- LOGNO
